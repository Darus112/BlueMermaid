...
export default function CartItem({item, setFlag, flag}) {
    ...
    return (
        ...
        <div className="flex flex-col gap-2">
            <p className="text-seagull-900 font-body font-light text-lg">{item?.title}</p>
            <p className="text-seagull-900 font-body font-normal text-sm text-left">${parseFloat(item?.price) * qty}</p>
        </div>
        ...
        <div className="group flex items-center gap-2 ml-auto cursor-pointer">
            ...
            items-center justify-center shadow-lg">{qty}</p>
            <motion.div 
                whileTap={{scale : 0.75}}
